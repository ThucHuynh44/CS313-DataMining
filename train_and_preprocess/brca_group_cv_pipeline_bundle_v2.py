"""
BRCA Tumor vs Normal classification with nested Group-Stratified CV and hold-out test.

Bundle v3 (with correlation filter):
...
- save_inference_bundle(results, out_dir, prefix="brca") -> paths
- load_inference_bundle(out_dir, prefix="brca") -> (model, input_cols, selected_cols, metadata)
...
LABEL_MAP_DEFAULT = {"Normal": 0, "Tumor": 1}


@dataclass
class ExperimentConfig:
    MATRIX_PATH: str = "./brca_matrix_with_label.tsv"
    META_PATH: str = "./tumor_normal_label.csv"

    MODEL: str = "rf"  # "logreg_l2" | "linear_svm" | "rf" | "xgb"

    # Preprocess / feature selection
    K_FEATURE_LIST: Tuple[int, ...] | int = (50, 100, 500, 1000, 2000, 3000, 4000)
...
def load_data(matrix_path: str, meta_path: str):
    ...
    if y.isna().any():
        ...
        raise ValueError(f"Unexpected labels. Allowed {{'Tumor','Normal'}}. Found: {bad}")
...
def feature_importance_dataframe(clf, selected_names: List[str]) -> pd.DataFrame:
            ...
            pd.DataFrame(
                {
                    ...
                    "direction": np.where(coef >= 0, "↑ toward Tumor (1)", "↓ toward Tumor (1)"),
                }
            )
...
def run_brca_experiment(cfg: ExperimentConfig) -> Dict[str, Any]:
    X, y, groups, input_feature_cols, zero_frac = load_data(cfg.MATRIX_PATH, cfg.META_PATH)
    ...
    report = classification_report(
        y_test, y_pred, digits=4, target_names=["Normal (0)", "Tumor (1)"]
    )

    selected_feature_cols = extract_selected_feature_names(best_pipe, input_feature_cols)
    clf = best_pipe.named_steps["clf"]
    feat_df = feature_importance_dataframe(clf, selected_feature_cols)
...
def save_inference_bundle(results: Dict[str, Any], out_dir: str, prefix: str = "brca") -> Dict[str, str]:
    import joblib
...
def load_inference_bundle(out_dir: str, prefix: str = "brca"):
    import joblib
...
def print_summary(results: Dict[str, Any], top_n: int = 10) -> None:
    ...
    print(f"Phân bố nhãn: {ov['label_distribution']} (Tumor=1, Normal=0)")
    ...
    print("\n===== INNER CV (GridSearch) =====")
    print(f"best {results['config'].SCORING}={icv['best_score']:.4f}")
    ...
    print(pd.DataFrame(np.round(cmn, 4), index=["True 0", "True 1"], columns=["Pred 0", "Pred 1"]))
